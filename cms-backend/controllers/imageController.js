...
const Post = require('../models/post'); // 1) Import your Post model
...
async function uploadImage(req, res) {
  try {
    ...
    // Extract "my image.png" from "1690123456789-my image.png"
    ...
    let mainNamePart = originalFileName
      ...
      .replace(/[^a-zA-Z0-9-_]/g, ''); // Remove any other unsafe characters
    ...
    // Attempt dimension gathering if image or video
    ...
    // If image => get new compressed size
    ...
    const uploaderName = req.body.uploadedBy || 'Unknown';
...
async function getImages(req, res) {
  try {
    const images = await Image.find({})
      .sort({ createdAt: -1 })
      .select('name url size dimensions format type createdAt altText caption description title uploadedBy')
      .lean()
      .exec();

    // 1) For each image, count usage in Post by matching featureImage.url
    const imagesWithUsage = await Promise.all(
      images.map(async (img) => {
        // Count how many posts have featureImage.url === img.url
        const usageCount = await Post.countDocuments({ 'featureImage.url': img.url });
    ...
    res.status(200).json({ images: imagesWithUsage });
  } catch (error) {
