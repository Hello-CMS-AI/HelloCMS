...
function LiveBlogEditor() {
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        setLoading(true);
        const res = await fetch(`https://api.dinasuvadu.in/api/posts/${id}`);
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchLiveUpdates = async () => {
      try {
        const res = await fetch(`https://api.dinasuvadu.in/api/live-updates/${id}`);
        if (!res.ok) {
  ...
  const handleSaveEditUpdate = async ({ title, content, pinned }) => {
    ...
    try {
      const res = await fetch(
        `https://api.dinasuvadu.in/api/live-updates/${editUpdate._id}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteUpdate = async (updateId) => {
    if (!updateId) return;
    try {
      const res = await fetch(`https://api.dinasuvadu.in/api/live-updates/${updateId}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        const data = await res.json();
  ...
  const handleTogglePin = async (update) => {
    try {
      ...
      const res = await fetch(
        `https://api.dinasuvadu.in/api/live-updates/${update._id}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
