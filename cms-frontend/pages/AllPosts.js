...
const AllPosts = () => {
  ...
  useEffect(() => {
    // Distinct authors
    fetch('https://api.dinasuvadu.in/api/posts/distinct-authors')
      .then((res) => res.json())
      .then((data) => {
    ...
    // Categories
    fetch('https://api.dinasuvadu.in/api/categories/list-categories')
      .then((res) => res.json())
      .then((data) => {
        setCategories(data);
  ...
  const fetchPosts = async () => {
    try {
      ...
      const res = await fetch(`https://api.dinasuvadu.in/api/posts?${queryParams.toString()}`);
  ...
  const handleTrash = async (postId) => {
    try {
      const res = await fetch(`https://api.dinasuvadu.in/api/posts/${postId}`, {
        method: 'PUT',
