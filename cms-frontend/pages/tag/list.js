...
const ListTags = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`https://api.dinasuvadu.in/api/tags/delete-tag/${id}`, {
        method: 'DELETE',
      });
  ...
  const handleTrendingToggle = async (id) => {
    try {
      const response = await fetch(`https://api.dinasuvadu.in/api/tags/mark-as-trending/${id}`, {
        method: 'PUT',
      });
